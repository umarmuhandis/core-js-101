...
//   // throw new Error('Not implemented');
//   const strArr = str.replace(' ', '').trim().split('');

//   const obj = {};

//   for (let i = 0; i < strArr.length; i += 1) {
//     if (!Object.prototype.hasOwnProperty.call(obj, strArr[i])) {
//       obj[strArr[i]] = 1;
//     } else {
//       obj[strArr[i]] += 1;
//     }
...
//   return keys[values.indexOf(1)] ? keys[values.indexOf(1)] : null;
// }
...
function isCreditCardNumber(ccn) {
  ...
  let oddSum = 0;
  numArr.slice().forEach((num, index) => {
    if (index % 2 === 1) {
      oddSum += Number(num);
    }
  });
  ...
  for (let i = 0; i < numArr.length; i += 2) {
    const currentNumberDoubled = Number(numArr[i]) * 2;
    if (currentNumberDoubled >= 10) {
      evenSum += currentNumberDoubled - 9;
    } else {
      evenSum += currentNumberDoubled;
    }
  }
